class PromptGenerator {
    ...
    setupEventListeners() {
        ...
        form.addEventListener('submit', (e) => this.handleGeneratePrompt(e));

        const testBtn = document.getElementById('testPrompt');
        ...
        // Setup model input autocomplete
        const modelInput = document.getElementById('model');
        const modelSuggestions = document.getElementById('modelSuggestions');
        
        modelInput.addEventListener('input', (e) => this.handleModelInput(e));
        modelInput.addEventListener('focus', () => this.showModelSuggestions());
        modelInput.addEventListener('blur', () => {
            // Delay hiding suggestions to allow clicking on them
            setTimeout(() => this.hideModelSuggestions(), 150);
        });

        // Load current settings values when page loads
        this.loadCurrentSettings();
    }

    async handleGeneratePrompt(e) {
        e.preventDefault();
        ...
        try {
            const prompt = await this.generatePrompt(task, variables);
            this.currentPromptTemplate = this.extractPromptTemplate(prompt);
            this.displayGeneratedPrompt(this.currentPromptTemplate);

            // Add to history
            await this.addToHistory(task, this.currentPromptTemplate);
    ...
    async generatePrompt(task, variables) {
        const metaprompt = this.getMetaprompt();
        const userMessage = metaprompt.replace('{{TASK}}', task);
    ...
    extractPromptTemplate(response) {
        const instructionsMatch = response.match(/<Instructions>(.*?)(?:<\/Instructions>|$)/s);
        if (instructionsMatch) {
            let prompt = instructionsMatch[1].trim();
            // Clean up empty tags and formatting
            prompt = prompt.replace(/\n<(\w+)>\s*<\/\1>\n/g, '');
            return prompt;
        }
        return response; // Fallback to full response if parsing fails
    }
    ...
    displayGeneratedPrompt(prompt) {
        ...
        messageDiv.innerHTML = '<div class="success">✨ Prompt generated successfully!</div>';
    ...
    async handleTestPrompt() {
        if (!this.currentPromptTemplate) {
            this.showError('Please generate a prompt template first');
            return;
        }
        ...
        this.showLoading(true, 'Testing prompt...');

        try {
            const result = await this.testPrompt(this.currentPromptTemplate, task);
            this.displayTestResult(result);

        } catch (error) {
            this.showError('Failed to test prompt: ' + error.message);
        }

        this.showLoading(false);
    }

    async testPrompt(promptTemplate, originalTask) {
        // Create test inputs based on the task
        const testInputs = this.generateTestInputs(originalTask);
    ...
    generateTestInputs(task) {
        const inputs = {};

        // Simple heuristic to generate test inputs based on common variables
        if (task.toLowerCase().includes('email') && task.toLowerCase().includes('customer')) {
            inputs['CUSTOMER_COMPLAINT'] = 'The product arrived damaged and the customer service was unresponsive.';
            inputs['COMPANY_NAME'] = 'TechSolutions Inc.';
        } else if (task.toLowerCase().includes('menu')) {
            inputs['MENU'] = 'Pizza ($15), Burger ($12), Salad ($10), Pasta ($14)';
            inputs['PREFERENCES'] = 'Vegetarian, no spicy food, budget under $15';
        } else {
            inputs['INPUT1'] = 'Sample input text';
            inputs['INPUT2'] = 'Another sample input';
        }
    ...
    displayTestResult(result) {
        ...
        messageDiv.innerHTML = '<div class="success">🧪 Test completed successfully!</div>';
    ...
    toggleSettings() {
        ...
        // Load current settings into form when opening
    ...
    async saveSettings() {
        ...
        // Add to model history if it's a new model
        if (modelValue && !this.modelHistory.includes(modelValue)) {
            this.modelHistory.unshift(modelValue);
            // Keep only last 10 models
            this.modelHistory = this.modelHistory.slice(0, 10);
            await this.saveModelHistory();
        }
    ...
    loadCurrentSettings() {
        // Load settings into form fields immediately
        document.getElementById('apiUrl').value = this.settings.apiUrl;
        document.getElementById('apiKey').value = this.settings.apiKey;
        document.getElementById('model').value = this.settings.model;
    }
    ...
    handleModelInput(e) {
        ...
        const suggestions = this.getModelSuggestions(value);
        this.displayModelSuggestions(suggestions);
    }

    getModelSuggestions(input) {
        const allModels = [...this.defaultModels, ...this.modelHistory];
        const uniqueModels = [...new Set(allModels)];
        
        if (!input.trim()) {
            return uniqueModels.slice(0, 5);
        }
    ...
    displayModelSuggestions(suggestions) {
        ...
        suggestionsDiv.innerHTML = suggestions.map(model => 
            `<div class="model-suggestion" onclick="app.selectModel('${model}')">${model}</div>`
        ).join('');
    ...
    showModelSuggestions() {
        ...
        const suggestions = this.getModelSuggestions(input.value.toLowerCase());
        this.displayModelSuggestions(suggestions);
    }
    ...
    async saveModelHistory() {
        try {
            await window.electronAPI.saveModelHistory(this.modelHistory);
        } catch (error) {
            console.error('Failed to save model history:', error);
        }
    }

    async loadModelHistory() {
        try {
            const result = await window.electronAPI.loadModelHistory();
            if (result.success) {
                this.modelHistory = result.data || [];
            }
        } catch (error) {
    ...
    async addToHistory(task, promptTemplate) {
        const historyItem = {
    ...
    updateHistoryList() {
        ...
        if (this.history.length === 0) {
            list.innerHTML = '<p style="color: #666; text-align: center;">No prompt history yet</p>';
            return;
        }

        list.innerHTML = this.history.slice(0, 10).map(item => `
            <div class="history-item" onclick="app.loadFromHistory('${item.id}')">
                <h4>${this.truncateText(item.task, 50)}</h4>
                <p>${this.truncateText(item.prompt, 100)}</p>
                <div class="date">${this.formatDate(item.timestamp)}</div>
            </div>
        `).join('');
    }
    ...
    loadFromHistory(id) {
        ...
        if (item) {
            ...
            document.getElementById('variables').value = item.variables;
            this.currentPromptTemplate = item.prompt;
            this.displayGeneratedPrompt(item.prompt);
        }
    }
    ...
    toggleTheme() {
        ...
        // Save theme preference to local storage if available
        try {
    ...
    // Metaprompt based on the reference implementation
    getMetaprompt() {
...
Note: If you want the AI to output its entire response or parts of its response inside certain tags, specify the name of these tags (e.g. "write your answer inside <answer> tags") but do not include closing tags or unnecessary open-and-close tag sections.`;
    ...
    showLoading(loading, message = 'Generating prompt...') {
